...
class Subsite extends DataObject {
	...
	public static function writeHostMap($file = null) {
		...
		// Add main url to hostmap if not defined as default subsite in CMS
		if(!isset($hostmap['default'])) {
			$baseURL = Config::inst()->get('FilesystemPublisher', 'static_base_url');
			// Strip protocol and trailing slash
			preg_match('%//(.*)%', $baseURL, $matches);
			if (array_key_exists(1, $matches)) {
				$mainHost = trim($matches[1],'/');
				$hostmap[$mainHost] = $mainHost;
			}
		}
